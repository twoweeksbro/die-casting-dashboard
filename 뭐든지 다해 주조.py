...
with col1:
    ...
    st.page_link("pages/2_불량 데이터 분석 대시보드.py", label="불량 분석 대시보드")

with col2:
    st.page_link("pages/3_과거 데이터 분석 데스크.py", label="과거 데이터 분석 데스크")
    st.page_link("pages/4_불량 확률 예측 시뮬레이터.py", label="불량 확률 예측 시뮬레이터")
    st.page_link("pages/5_공지 사항.py", label="공지사항")
